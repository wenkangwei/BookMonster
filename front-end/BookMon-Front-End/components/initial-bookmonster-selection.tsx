...
interface InitialBookMonsterSelectionProps {
  onSelectBookMonster: (bookMonster: BookMonster) => void
}

export function InitialBookMonsterSelection({ onSelectBookMonster }: InitialBookMonsterSelectionProps) {
  const [selectedBookMonster, setSelectedBookMonster] = useState<BookMonster | null>(null)
  const [starterBookMonsters, setStarterBookMonsters] = useState<BookMonster[]>([])
  const [isLoading, setIsLoading] = useState(true)
  const [error, setError] = useState<string | null>(null)

  useEffect(() => {
    const loadStarterBookMonsters = async () => {
      try {
        setIsLoading(true)
        setError(null)
        console.log("Loading starter book monsters from API...")

        const bookMonsters = await GameApiService.initMonster()
        console.log("Loaded starter book monsters:", bookMonsters)

        setStarterBookMonsters(bookMonsters)
      } catch (error) {
        console.error("Failed to load starter book monsters:", error)
        setError("加载初始书籍怪兽失败，请重试")
      } finally {
        setIsLoading(false)
      }
    }

    loadStarterBookMonsters()
  }, [])
      ...
      <div className="h-full w-full bg-gradient-to-b from-blue-400 to-green-400 flex items-center justify-center">
        <Card className="w-96 text-center bg-white/95 backdrop-blur">
          <CardContent className="p-8">
            ...
            <p className="text-gray-600">正在从服务器获取初始书籍怪兽</p>
          </CardContent>
        </Card>
      </div>
    )
      ...
      <div className="w-full h-full max-w-4xl mx-auto flex flex-col">
        ...
        <div className="text-center py-2 portrait:py-4 landscape:py-1 flex-shrink-0">
          ...
          <p className="text-sm portrait:text-base landscape:text-sm text-white/80 mt-1">
            从服务器加载了 {starterBookMonsters.length} 只书籍怪兽
          </p>
        </div>
        ...
        <div className="flex-1 flex flex-col min-h-0">
          ...
          <div className="flex-1 flex items-center justify-center min-h-0 px-2">
            <div className="w-full max-w-3xl">
              <div className="grid grid-cols-1 portrait:grid-cols-1 landscape:grid-cols-3 gap-3 portrait:gap-4 landscape:gap-2">
                {starterBookMonsters.map((bookMonster) => (
                  <div
                    key={bookMonster.id}
                    className={`cursor-pointer transition-all ${
                      selectedBookMonster?.id === bookMonster.id ? "ring-2 ring-yellow-400 ring-offset-2" : ""
                    }`}
                    onClick={() => setSelectedBookMonster(bookMonster)}
                  >
                      ...
                      <CardContent className="p-3 portrait:p-4 landscape:p-2">
                        <div className="flex portrait:flex-row landscape:flex-col items-center portrait:gap-4 landscape:gap-2">
                          ...
                          <div className="flex-1 portrait:text-left landscape:text-center">
                            <h3 className="text-base portrait:text-lg landscape:text-sm font-bold">
                              {bookMonster.name}
                            </h3>
                            <p className="text-sm portrait:text-base landscape:text-xs text-gray-600">
                              等级 {bookMonster.level} • {bookMonster.attribute}属性
                            </p>
                            <div className="grid grid-cols-2 gap-1 text-xs portrait:text-sm landscape:text-xs mt-1 portrait:mt-2 landscape:mt-1">
                              <div>HP: {bookMonster.maxHp}</div>
                              <div>攻击: {bookMonster.stats.attack}</div>
                              <div>防御: {bookMonster.stats.defense}</div>
                              <div>速度: {bookMonster.stats.speed}</div>
                            </div>
                            {bookMonster.bookName && (
                              <p className="text-xs text-purple-600 mt-1">来自《{bookMonster.bookName}》</p>
                            )}
                          </div>
                        </div>
                      </CardContent>
              ...
              <Card className="bg-white/95 backdrop-blur">
                <CardContent className="p-3 portrait:p-4 landscape:p-2">
                  <div className="flex flex-col portrait:flex-row landscape:flex-row gap-3 portrait:gap-4 landscape:gap-2 items-center">
                    <img
                      src={selectedBookMonster.image || "/placeholder.svg"}
                      alt={selectedBookMonster.name}
                    ...
                    <div className="flex-1 text-center portrait:text-left landscape:text-left min-w-0">
                      ...
                      <p className="text-xs portrait:text-sm landscape:text-xs text-gray-700 mb-2 line-clamp-2 portrait:line-clamp-3 landscape:line-clamp-2">
                        {selectedBookMonster.description}
                      </p>
                      <div className="flex gap-2 text-xs">
                        <span className="bg-blue-100 px-2 py-1 rounded">{selectedBookMonster.attribute}属性</span>
                        <span className="bg-green-100 px-2 py-1 rounded">经验值: {selectedBookMonster.exp}</span>
                      </div>
                    </div>
                  </div>
                </CardContent>
          ...
          <div className="flex-shrink-0 p-2 portrait:p-3 landscape:p-1">
            <Button
              onClick={() => selectedBookMonster && onSelectBookMonster(selectedBookMonster)}
              disabled={!selectedBookMonster}
              size="lg"
            ...
            >
              {selectedBookMonster ? `确认选择 ${selectedBookMonster.name}` : "请选择一只书籍怪兽"}
            </Button>
          </div>
        </div>
      </div>
