...
export async function POST(request: NextRequest) {
  try {
    ...
    // 尝试调用后端API
    try {
      console.log("Attempting to call backend API at http://localhost:8004/init_monster")

      const controller = new AbortController()
      const timeoutId = setTimeout(() => controller.abort(), 10000) // 10秒超时

      const response = await fetch("http://localhost:8004/init_monster", {
        method: "POST",
        headers: {
      ...
      // 使用本地后备数据
      console.log("Generating fallback starter monsters...")
