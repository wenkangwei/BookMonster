...
class MonsterBrochure():
    ...
    def udpate_brochure(self, monster_info, save_path=None, save_mode="w+"):
        ...
        for monster_data in monster_info:
            self._monster_data[monster_data['name']] = monster_data
...
class MonsterState():
    def __init__(self, json_config) -> None:
        ...
        self.remain_question = {}
        for q in self.question_list:
            if "question" in q:
                self.remain_question[q["question"][0]] = q
        self.id = json_config.get("id", '0')
        self.monster_id = json_config.get("monster_id", 0)
        self.book_nm =json_config.get("book_nm", '')
    def update_currentHp(self, hp: int):
        self.currentHp = hp
    def set_remain_question(self, question_list):
        self.remain_question = { q["question"][0]: q for q in question_list}
...
class EnvironmentState():
    ...
    def update_player_state(self):
        ...
        return
    def udpate_brochure(self,):
        # self.monster_brochure.udpate_brochure()
        return
...
#         }
    ...
    def pick_question(monster_id, enemy_state, player_state):
        import random
        action_list = gb_state.enermy_monsters_states[monster_id].remain_question
        idx_ls = [i for i in range(action_list)]
